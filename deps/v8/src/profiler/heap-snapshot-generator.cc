...
void V8HeapExplorer::PopulateLineEnds() {
  ...
  CombinedHeapObjectIterator iterator(heap_, HeapObjectIterator::kFilterUnreachable);

  for (HeapObject obj = iterator.Next(); !obj.is_null(); obj = iterator.Next()) {
    JSFunction func = GetLocationFunction(obj);
    if (!func.is_null() && func.shared().script().IsScript()) {
      scripts.push_back(handle(Script::cast(func.shared().script()), isolate()));
    }
  }
